package com.order.api.configures.web;
...
public class SimplePageRequestHandlerMethodArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    public Object resolveArgument(
        ...
        long offset;
        int size;
        if(offsetString==null||Long.parseLong(offsetString)<0||Long.parseLong(offsetString)>Long.MAX_VALUE)
            offset = DEFAULT_OFFSET;
        else
            offset = Long.parseLong(offsetString);
        if(sizeString==null||Integer.parseInt(sizeString)<0||Integer.parseInt(sizeString)>5)
            size = DEFAULT_SIZE;
        else

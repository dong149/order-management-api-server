...
> 힌트: @ControllerAdvice 어노테이션이 적용되있는 GeneralExceptionHandler 구현을 참고하세요.

이미 구현되 있는 아래 2개 API는 응답 포맷이 `요건 1` 정의에 맞지 않기 때문에 수정이 필요합니다.

- 단일 상품 조회: /api/products/{id}
  - 구현위치: ProductRestController.findById 메소드
- 상품 목록 조회: /api/products
  - 구현위치: ProductRestController.findAll 메소드

그 외 API는 `요건 1`을 만족하도록 개발을 시작하세요.

> 힌트: UserRestController 구현을 참고하세요.
...
> 힌트: 페이징 파라미터 (offset, size) 처리를 위해 SimplePageRequestHandlerMethodArgumentResolver 클래스 완성이 필요합니다.
